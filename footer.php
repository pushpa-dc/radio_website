<div class="container-fluid">
    <div class="row footer-top">
        <div class="col-sm-4">One</div>
        <div class="col-sm-4">Two</div>
        <div class="col-sm-4">Three</div>
    </div>
    <div class="row copyright pt-2 pb-2">
        <div class="col-sm-4">&copy;<?php echo date('Y').' '; bloginfo( 'name' );?></div>
        <div class="col-sm-4"></div>
        <div class="col-sm-4"><a href="http://www.appharu.com?ref=demo">By appharu.com</a></div>
    </div>
</div>
</div><!-- container-fluid -->
<?php wp_footer();?>
</body>
</html>

<div id="live">
    <a href="" target="_blank"><img src="<?php echo get_stylesheet_directory_uri() . '/img/radio.png'; ?>" alt=""></a>
</div>
<div class="circleanimation"></div>
